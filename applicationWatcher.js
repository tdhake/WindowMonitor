...
const pfnWinEventProc = ffi.Callback(
	...
	function (
		...
		try {
			...
			if (EVENT_SYSTEM_CAPTURESTART == event && finallyWindowTitle != 'Running applications'){
				if (prevPID != retData.pid) {
					prevPID = retData.pid;
					appSwitched = true;
					console.log("CAPTURESTART - app is switched and previous pid is current pid", prevPID);
				} else {
					appSwitched = false;
					// console.log('app is not switched, current pid: ', retData.pid);
				}
			}
			...
			if (
				...
				// prevPID = retData.pid;
				console.debug(displayCurrentTime(), 'app is not switched and Window size/position is changed, postMessage called', retData);
				// parentPort.postMessage(JSON.stringify(retData));
			} else if (EVENT_SYSTEM_FOREGROUND == event) {
				console.log('isPosChanged', isPosChanged(activeWindowDimensions));
				console.debug(displayCurrentTime(), 'app is switched, Foreground app is changed, postMessage called', retData);
				// parentPort.postMessage(JSON.stringify(retData));
			}
		} catch (e) {
			console.error(e);
